...
}); paging(n.total, n.count, n.currentPage)
}
}).fail(function (n) { if (n.status === 403) console.log(n), alert("You are not authorized you to access logs.\r\nYou are not logged in or you don't have enough permissions to perform the requested operation."); else if (n.status === 500) { const t = JSON.parse(n.responseJSON.errorMessage); alert(t.errorMessage) } else alert(n.responseText) })
}, levelClass = n => { switch (n) { case "Verbose": case "Debug": return "bg-success"; case "Information": return "bg-primary"; case "Warning": return "bg-warning"; case "Error": return "bg-danger"; case "Error": return "bg-danger"; case "Critical": return "bg-danger"; default: return "" } },
    formatDate = n => {
        var t = new Date(n); return `${(t.getMonth() + 1).toString().padStart(2, "0")}/${t.getDate().toString().padStart(2, "0")}/${t.getFullYear().toString().padStart(4, "0")}
